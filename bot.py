...
TEAMS={"arsenal":"Arsenal","aston villa":"Aston Villa","man city":"Man City","liverpool":"Liverpool","chelsea":"Chelsea","man united":"Man United","tottenham":"Tottenham","newcastle":"Newcastle","brighton":"Brighton","west ham":"West Ham","crystal palace":"Crystal Palace","fulham":"Fulham","wolves":"Wolves","everton":"Everton","nottm forest":"Nottm Forest","brentford":"Brentford","bournemouth":"Bournemouth","real madrid":"Real Madrid","barcelona":"Barcelona","ath madrid":"Ath Madrid","atletico":"Ath Madrid","sevilla":"Sevilla","betis":"Betis","villarreal":"Villarreal","real sociedad":"Real Sociedad","ath bilbao":"Ath Bilbao","rayo vallecano":"Rayo Vallecano","rayo":"Rayo Vallecano","valencia":"Valencia","celta":"Celta","mallorca":"Mallorca","osasuna":"Osasuna","girona":"Girona","getafe":"Getafe","alaves":"Alaves","leganes":"Leganes","espanyol":"Espanyol","valladolid":"Valladolid","inter":"Inter","ac milan":"AC Milan","milan":"AC Milan","juventus":"Juventus","napoli":"Napoli","roma":"Roma","lazio":"Lazio","atalanta":"Atalanta","bayern":"Bayern Munich","dortmund":"Dortmund","leverkusen":"Leverkusen","psg":"Paris SG","marseille":"Marseille","lille":"Lille","monaco":"Monaco","lyon":"Lyon","ajax":"Ajax","psv":"PSV","feyenoord":"Feyenoord","benfica":"Benfica","porto":"Porto","sporting":"Sp Lisbon","america":"America","chivas":"Guadalajara","guadalajara":"Guadalajara","monterrey":"Monterrey","tigres":"Tigres UANL","cruz azul":"Cruz Azul","pumas":"Pumas","toluca":"Toluca","inter miami":"Inter Miami","lafc":"Los Angeles FC","la galaxy":"LA Galaxy","flamengo":"Flamengo","palmeiras":"Palmeiras","boca juniors":"Boca Juniors","river plate":"River Plate"}
...
def detect_liga(text):
    ...
    if any(x in t for x in ["america","chivas","guadalajara","monterrey","tigres","cruz azul","pumas","toluca"]): return "MEX_PARCIAL"
    if any(x in t for x in ["miami","lafc","galaxy"]): return "MLS_PARCIAL"
...
def h(m):
    ...
    if "PARCIAL" in liga:
        s1=get_stats_europe(l,"SP1"); s2=get_stats_europe(v,"SP1")
        txt=f"*{s1['name']} vs {s2['name']}* - {liga}\n⚠️ DATOS PARCIALES\n\nTiros: {s1['shots']} / {s2['shots']}\nCorners: {s1['corn']} / {s2['corn']}\nTarjetas: {s1['cards']} / {s2['cards']}"
    else:
        s1=get_stats_europe(l,liga); s2=get_stats_europe(v,liga)
        txt=f"*{s1['name']} vs {s2['name']}* - {s1['liga']} Ult {s1['n']}\n✅ DATOS COMPLETOS\n\nGol 1T: {s1['ht']}% | {s2['ht']}% -> *{int((s1['ht']+s2['ht'])/2)}%*\nCorner 1T: {s1['corn_ht']} | {s2['corn_ht']}\n\nOver1.5 {int((s1['o15']+s2['o15'])/2)}% Over2.5 {int((s1['o25']+s2['o25'])/2)}%\nCorners: {s1['corn']} / {s2['corn']}\nTarjetas: {s1['cards']} / {s2['cards']}\nTiros: {s1['shots']} / {s2['shots']}\nTiros puerta: {s1['sot']} / {s2['sot']}\nBTTS {int((s1['btts']+s2['btts'])/2)}%\n\nFuente: {s1['src']}"
    bot.reply_to(m, txt, parse_mode="Markdown")

print("V12.2 LISTO", flush=True)
